    ...
    public class ChartDataController : ApiController
    {
        ...
        public DataSet CaseResolutionData(PassingParam passingParam)
        {
            ...
            try
            {
                ...
                DataSet dsData = ExecuteProc(parametes, "P_ChartData_MyOSS_Cases", "CaseData", true);
                //DataSet dsData = ExecuteProc(parametes, "P_ChartData_MyOSS_Cases", "CaseData", true);

                if (dsData != null && dsData.Tables.Count > 0 && dsData.Tables[2].Rows[0]["Result"].ToString() == "Y")
                {
                    //"<a class=edit-ProjectBCDocument data-url="+entry.BCAttachmentID+" href=#>"+entry.BCAttachment+"</a>"
                    ...
                    foreach (DataRow currRow in dtWeeks.Rows)
                    {
                        ...
                        //dr["WeekDesc"] += "'" + "<a class=edit-Week data-url=" + currRow["WeekDesc"].ToString().Trim() + " href=#>" + currRow["WeekDesc"].ToString().Trim() + "</a>" + "'" + ",";
                        ...
                        //drTblHeader["title"] = "<a class=edit-Week data-url=" + currRow["WeekDesc"].ToString().Trim() + " href=#>" + currRow["WeekDesc"].ToString().Trim() + "</a>";
        ...
        public DataSet CaseResolutionApplicationData(PassingParam passingParam)
        {
            ...
            try
            {
                if (passingParam.Param3 != "")
                passingParam.Param3 = passingParam.Param3.Remove(0, 2).Trim();
                Object[] parametes = new Object[] 
                    {
                        passingParam.Param1,
                        passingParam.Param2.Length >0 ?int.Parse(passingParam.Param2):0,
                        passingParam.Param3.Length >0 ?int.Parse(passingParam.Param3):0
                    };
        ...
        [HttpPost]
        public DataSet CaseResolutionDepartmentData(PassingParam passingParam)
        {
            DataSet dsToReturn = new DataSet();
            try
            {
                ...
                DataSet dsData = ExecuteProc(parametes, "P_ChartData_MyOSS_Cases_Department", "CaseData", true);
                ...
                if (dsData != null && dsData.Tables.Count > 0 && dsData.Tables[2].Rows[0]["Result"].ToString() == "Y")
                {
                    ...
                    DataView dvWeeks = new DataView(dsData.Tables[0]);
                    DataTable dtWeeks = dvWeeks.ToTable(true, "WeekNo", "DepartmentName");
                    DataTable dtTableData = new DataTable();
                    DataView dvData = new DataView(dsData.Tables[0]);
                    DataTable dtDistinctRows = dvData.ToTable(true, "RowNo", "RowDesc");
                    ...
                    foreach (DataRow currRow in dtWeeks.Rows)
                    { 
                        dr["WeekDesc"] += "'" + currRow["DepartmentName"].ToString().Trim() + "'" + ",";
                        dtTableData.Columns.Add("Week" + lpCtr.ToString());
                        drTblHeader = dtTblHeader.NewRow();
                        drTblHeader["title"] = currRow["DepartmentName"].ToString().Trim();
